...
def k_fold_cross_validation():
    ...
    x = data[:, 0:-1]
    y = data[:,-1]
    kfold = min(min([ np.size(np.where(y==c)) for c in range(min_class_label,max_class_label+1)]),5)
    ...
    for train_index, test_index in skf:
        ...
        """for t in train_results.keys():
            test(X_train, y_train, train_results[t]['weights'])
            test(X_train, y_train, train_results[t]['weights_mv'])"""
        test_id += 1
