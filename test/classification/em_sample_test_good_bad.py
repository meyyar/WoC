...
no_of_expert_groups = [20]#[20,40,60,80,100,120,140,160,180,200]
expert_good_bad = [1.0]#[ 0.1,0.2,0.3, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90] 
...
def k_fold_cross_validation():
    ...
    for train_index, test_index in skf:
        ...
        for t in train_results.keys():
            for ex_goods in train_results[t].keys(): 
                test_results[test_id].update({ str(t)+str(ex_goods)+'EM' : test(X_test, y_test, train_results[t][ex_goods]['weights']) })
                test_results[test_id].update({ str(t)+str(ex_goods)+'MV' : test(X_test, y_test, train_results[t][ex_goods]['weights_mv']) })
                test_results[test_id].update({ str(t)+str(ex_goods)+'AT' : test(X_test, y_test, train_results[t][ex_goods]['weights_at']) })
...
def plot_graph(ax, test_results, kfold, type, color, legendpos):
    ...
    plot_x = expert_good_bad
    plot_accerr = np.std([[test_results[k][str(exg)+str(g)+type][0]  for exg in range(len(no_of_expert_groups)) for  g in expert_good_bad ]for k in range(kfold) ],axis=0)
    plot_y = np.mean([[test_results[k][str(exg)+str(g)+type][1] for exg in range(len(no_of_expert_groups)) for  g in expert_good_bad ] for k in range(kfold) ] ,axis=0)
    plot_yerr = np.mean([[test_results[k][str(exg)+str(g)+type][2]  for exg in range(len(no_of_expert_groups)) for  g in expert_good_bad ] for k in range(kfold) ],axis=0)
    plot_acc = np.mean([[test_results[k][str(exg)+str(g)+type][0]  for exg in range(len(no_of_expert_groups)) for  g in expert_good_bad ] for k in range(kfold) ],axis=0)
    ax.set_xlim(plot_x[0]-0.1, plot_x[-1]+0.1)
    ax.set_xticks(plot_x)
    handle.append(ax.errorbar(plot_x, plot_y, yerr=plot_yerr, fmt=color[0], ms=10)) #'ro'
    handle.append(ax.errorbar(plot_x, plot_acc, yerr=plot_accerr, fmt=color[1], ms=5)) #'bs'  
